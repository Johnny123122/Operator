...
client.on("message", async message => {
  ...
  if (message.content.startsWith(prefix12)) {
    ...
    if (blacklist121 === "Blacklisted")
      return message.reply(
        "You're blacklisted from using Operator by my developer!"
      );
    switch (command) {
      case "blacklist":
      ...
      case "repeat":
        if (args.includes("@everyone"))
          return message.reply(
            "I won't repeat a message with a here ping in it!"
          );
      ...
      case "giveaway":
        ...
        setTimeout(() => {
          if (m.reactions.cache.get("🎉").count <= 1) {
            message.channel.send(
              `Reactions: ${m.reactions.cache.get("🎉").count}`
            );
            return message.channel.send(
      ...
      case "feedback":
        let feedback_message = message.content
          .split(`${prefix12}feedback `)
          .join("");
        if (message.member.id == 656978020933959727)
          return message.reply(
            `You've been blacklisted from using this command by official bot developers!`
          );
        if (!feedback_message)
          return message.reply(
      ...
      case "role":
        ...
        break;
      case "report":
        if (message.member.id == 656978020933959727) return message.delete();
      ...
      case "howgay":
        ...
        if (!guytorate)
          return message.channel.send(
            "You never mentioned who I need to gayrate!"
          );
      ...
      case "ban":
        ...
        const target =
          message.mentions.members.first() || client.guilds.cache.get(args[0]);
      ...
      case "unban":
        ...
        message.guild.fetchBans().then(bans => {
          ...
          if (!bUser)
            return message.reply(`The User ID you entered isn't valid!`);
          message.guild.members.unban(bUser.user);
          const unbanconfirmation = new MessageEmbed()
            .setTitle(
              `${
                client.users.cache.get(userID).tag
              } has sucessfully been unbanned!`
            )
            .setColor("GREEN");
      ...
      case "help":
        ...
        let announcement =
          "Want help on a specific command? Use `ops!help <command>` for info within the given command!";
        let embed = new MessageEmbed()
        ...
        if (args[0].toLowerCase() == "extra")
          ...
          if (
            ...
            embed.setTitle(`COMMAND - ${command}`).setColor("GREEN");
