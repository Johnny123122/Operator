...
module.exports.run = async (bot, message, args) => {
  ...
  if (args[0] == "fun") return message.channel.send(embed121212112121);
  if (args[0] == "moderation") return message.channel.send(embed121);
  if (args[0] == "extra");
  ...
  sentembed
    ...
    .on(
      "collect",
      async () =>
        (await sentembed.edit(embed121)) &&
        sentembed.reactions.resolve("▶").users.remove(message.author.id)
    );
  ...
  sentembed
    ...
    .on(
      "collect",
      async () =>
        (await sentembed.edit(embed121212112121)) &&
        sentembed.reactions.resolve("⏭").users.remove(message.author.id)
    );
  ...
  sentembed
    ...
    .on(
      "collect",
      async () =>
        (await sentembed.edit(embedextra)) &&
        sentembed.reactions.resolve("◀").users.remove(message.author.id)
    );
};
module.exports.help = {
